...
void CustomizedUIRecordMgr::onRecording2MP4Done(bool bsuccess, int iResult, const wchar_t* szPath)
{
	...
	if (bsuccess){
		...
		::MessageBox(NULL, szMsg, _T("Pure Sonic"), MB_OK);
	}else {
	...
	case ZOOM_SDK_NAMESPACE::Recording_Start:	
		{
			...
			{
				::MessageBox(NULL, _T("local recording start"), _T("Pure Sonic"), MB_OK);
			}
		}
		break;
	case ZOOM_SDK_NAMESPACE::Recording_Stop:
		::MessageBox(NULL, _T("local recording stop"), _T("Pure Sonic"), MB_OK);
		break;
	case ZOOM_SDK_NAMESPACE::Recording_Pause:
		::MessageBox(NULL, _T("local recording pause"), _T("Pure Sonic"), MB_OK);
		break;
	case ZOOM_SDK_NAMESPACE::Recording_Connecting:
		::MessageBox(NULL, _T("local recording connecting"), _T("Pure Sonic"), MB_OK);
		break;
	case ZOOM_SDK_NAMESPACE::Recording_DiskFull:
		::MessageBox(NULL, _T("local recording diskfull"), _T("Pure Sonic"), MB_OK);
		break;
	}
}
...
void CustomizedUIRecordMgr::onCloudRecordingStatus(ZOOM_SDK_NAMESPACE::RecordingStatus status)
{
	...
	::MessageBox(NULL, wstrMsg.c_str(), _T("Pure Sonic"), MB_OK);
}

void CustomizedUIRecordMgr::onRecordPriviligeChanged(bool bCanRec)
{
	if (bCanRec)
		::MessageBox(NULL, _T("you can record now."),     _T("Pure Sonic"), MB_OK);
	else 
		::MessageBox(NULL, _T("you can not record now."), _T("Pure Sonic"), MB_OK);
}
