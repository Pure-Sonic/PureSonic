...
void CCustomizeInMeetingUIMgr::CreateChatWindow()
{
	if(NULL == m_pChatUIMgr)
	{
		...
		if(m_pChatUIMgr)
		{
			m_pChatUIMgr->Create(NULL, _T("Pure Sonic"), UI_WNDSTYLE_DIALOG, WS_EX_WINDOWEDGE);
...
void CCustomizeInMeetingUIMgr::Start()
{
	do 
	{
		...
		if (myfullScreen) {
			create_owner_hwnd(this, ScreenX, ScreenY - 20);
		}
		else {
...
void CCustomizeInMeetingUIMgr::UpdateMeetingTitle(bool bInWaitingRoom)
{
	...
	{
		if(bInWaitingRoom)
			swprintf_s(strTitle, title_len,_T("Pure Sonic -- You are in waiting room now"));   
		else if(0 != iMeetingID)
			swprintf_s(strTitle, title_len,_T("Pure Sonic -- meeting ID %I64u"), iMeetingID);   
		else
			swprintf_s(strTitle, title_len,_T("Pure Sonic"));				
...
void CCustomizeInMeetingUIMgr::ShowHideMoreMenuWindow(int x, int y)
{
	if (m_more_menu_ui == NULL)
	{
		...
		m_more_menu_ui->Create(m_hOwnerHwnd, _T("Pure Sonic"), UI_WNDSTYLE_DIALOG, WS_EX_WINDOWEDGE|WS_EX_TOOLWINDOW);
